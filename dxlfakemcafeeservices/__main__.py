...
from .app import FakeMcAfeeServices
...
if len(sys.argv) != 2:
    print "Usage: dxlfakemcafeeservices <configuration files directory>"
    sys.exit(1)
...
logging_config_path = os.path.join(config_dir, FakeMcAfeeServices.LOGGING_CONFIG_FILE)
...
with FakeMcAfeeServices(sys.argv[1]) as app:
    try:
        # Run the application
        app.run()
        running = True
